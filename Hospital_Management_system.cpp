    ...
    void Display()
{
    ...
    ifstream inFile;
    inFile.open("doctor_records.txt");
    string line;
    ...
    if (n == 0) {
        ...
        if (status) {
            ...
            ofstream outFile;
            outFile.open("doctor_records.txt", ios::app);

            // Write the record to the file
            outFile << "1.Doctor's ID                 :" << arr[i].id << "\n";
            outFile << "2.Doctor's Name               :" << arr[i].name << "\n";
            outFile << "3.Doctor's Age                :" << arr[i].age << "\n";
            outFile << "4.Doctor's Qualification      :" << arr[i].Q << "\n";
            outFile << "5.Doctor's Specialization     :" << arr[i].special << "\n";
            outFile << "6.Doctor's Experience         :" << arr[i].exp << "\n";
            outFile << "8.Doctor's city               :" << arr[i].city << "\n\n";
    ...
    void doctor_detail()
    {
        ifstream inFile("doctor_details.txt");
        if (inFile.is_open())
        {
            // Read data from the file and display it on the console
            string line;
            while (getline(inFile, line))
            {
                cout << line << "\n";
        ...
        else
        {
            ...
            cout << "\n\n";
            ofstream outfile;
            outfile.open("doctor_details.txt", ios::app);
            outfile << "ID"
                    << "\t \t"
                    << "Name"
                    << "\t \t"
            ...
            for (i = 0; i < docid; i++)
            {
                outfile << arr[i].id << "\t \t" << arr[i].name << "\t \t" << arr[i].special << "\t \t \t" << arr[i].Q << "\t  \t \t" << arr[i].age << "\n\n\n";
            }
            outfile.close();
            for (i = 0; i < docid; i++)
            {
            ...
            cout << "\nPress any key to choose another option...";
            //  "Doctor details written to file 'doctor_details.txt'.
        }
    ...
    void tot_no_of_doc()
{
    ...
    // Write the number of doctors to a file
    ofstream outFile;
    outFile.open("doctor_count.txt", ios::app);
    outFile << "Total Doctor's in Hospital: " << i << "\n";
    outFile.close();
    
    // Read the number of doctors from a file
    ifstream inFile;
    inFile.open("doctor_count.txt");
    if (inFile)
    {
        string line;
        while (getline(inFile, line))
...
void patient::Display()
{
    ...
    else
    {
        ...
        if (status == 1)
        {
            ...
            // File handling
            ofstream outFile;
            outFile.open("patient_records.txt", ios::app);
            outFile << "1.Patient's ID                                      :" << arr[i].id1 << "\n";
            outFile << "2.Patient's Name                                    :" << arr[i].name << "\n";
            outFile << "3.Patient's Age                                     :" << arr[i].age << "\n";
            outFile << "4.Patient's  Disease                                :" << arr[i].dis << "\n";
            outFile << "5.Patient's Symptoms                                :" << arr[i].sym << "\n";
            outFile << "6.Patient's Room No.                                :" << arr[i].room << "\n";
            outFile << "7.Patient's condition Before Admit                  :" << arr[i].con << "\n";
            outFile << "8.Patient's ADMIT Date                              :" << arr[i].date << "\n";
            outFile << "9.Patient's Room Charge                             :" << arr[i].charg << "\n";
            outFile << "10.Patient's Medicine charge                        :" << arr[i].bill << "\n";
            outFile << "\n";
            outFile.close();
        }
        else
...
void patient::patient_report()
{
    ...
    if (status)
    {
        ...
        // open a file in write mode and write the report details to it
        ofstream reportFile;
        reportFile.open("patient_reports.txt", ios::app);
        reportFile << "Patient ID: " << arr[i].id1 << "\n";
        reportFile << "Patient Name: " << arr[i].name << "\n";
        reportFile << "Patient Age: " << arr[i].age << "\n";
        reportFile << "Patient Symptoms: " << arr[i].sym << "\n";
        reportFile << "Patient Disease: " << arr[i].dis << "\n";
        reportFile << "Patient Admit Date: " << arr[i].date << "\n";
        reportFile << "Patient condition At The Time Of Discharge: " << arr[i].con << "\n\n";
        reportFile.close();
    }
    else
    {
...
{
    ifstream inFile("Patient_details.txt");
    if (inFile.is_open())
    {
        // Read data from the file and display it on the console
        string line;
    ...
    else
    {
        ...
        ofstream outfile;
        outfile.open("Patient_details.txt", ios::app);
        outfile << "ID"
                << "\t \t"
                << "illness"
                << "\t \t"
        ...
        for (i = 0; i < docid1; i++)
        {
            outfile << arr[i].id1 << "\t \t" << arr[i].dis << "\t \t \t" << arr[i].date << "\t  \t \t" << arr[i].age << "\n";
        }
        outfile.close();
    }
    getch();
...
void patient::tot_no_of_pat()
{
    ...
    // Create a file stream object to read patient count from file
    ifstream countFile("patient_count.txt");
    int i;
    countFile >> i;
    countFile.close();

    // Create a file stream object to read patient records from file
    ifstream fin("patients.txt");

    int patientCount = 0;
    while (fin >> arr[patientCount].id1 >> arr[patientCount].name >> arr[patientCount].age >> arr[patientCount].sym >> arr[patientCount].dis >> arr[patientCount].date >> arr[patientCount].con >> arr[patientCount].bill >> arr[patientCount].charg)
    {
        patientCount++;
    }
    ...
    cout << "Total Patients in Hospital    :   " << patientCount << "\n";
    cout << "Total Patients in File        :   " << i << "\n";
    cout << "\nPress Any KEY To choose another Option.... ";
...
void patient::gen_pat_report()
{
    ...
    // Create a file stream object to read patient records from file
    ifstream fin("patient_records.txt");

    int status = 0;
    for (i = 0; i < docid1; i++)
    {
        fin >> arr[i].id1 >> arr[i].name >> arr[i].age >> arr[i].sym >> arr[i].dis >> arr[i].date >> arr[i].con >> arr[i].bill >> arr[i].charg;
        if (arr[i].id1 == n)
        {
            status = 1;
            break;
    ...
    if (status)
    {
        ...
        // Create a file stream object to write patient bill to file
        ofstream fout("bills.txt", ios::app);
        fout << "Patient ID: " << arr[i].id1 << "\n";
        fout << "Patient Name: " << arr[i].name << "\n";
        fout << "Medicine Charge: " << arr[i].bill << "\n";
        fout << "Room Charge: " << arr[i].charg << "\n\n";
        fout.close();
